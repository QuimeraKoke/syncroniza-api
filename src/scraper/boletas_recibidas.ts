...
async function scrapeBallotsReceived(project: IProject) {
    ...
    try {
        ...
        await page.goto('https://loa.sii.cl/cgi_IMT/TMBCOC_MenuConsultasContribRec.cgi?dummy=1461943244650');
        ...
        for (const value of optionValues) {
            ...
            const tableSelector = 'body > div:nth-child(2) > center > table:nth-child(2) > tbody > tr:nth-child(6) > td > table';
                ...
                for (let i = 1; i < rowCount; i++) {
                    ...
                    if (await link.count() > 0) {
                        ...
                        if (href && !processedLinks.has(href)) {
                            ...
                            const table = page.locator('body > div:nth-child(2) > center > center > form:nth-child(2) > table');
                            ...
                            for (let i = 0; i < filteredRowIndices.length; i++) {
                                ...
                                let oversea = false;
                                    ...
                                    if(data[2] === 'VIG'){
                                        data[2] = 'Documento Recibido Vigente'
                                    } else {
                                        data[2] = 'Documento Recibido Anulado'
                                    }

                                    const realDate = new Date(Date.parse(formattedDate))
                                    const folioExists = await TransactionSchema.exists({externalID: data[2]});
                                    ...
                                    tableData.push({
                                        ...
                                        razon_social: data[5] || "",
                                        documento: 'BOLETA' || "",
                                        folio: 'R-' + 'B-' + data[4] + '-' + data[1] || "",
                                        fecha: formattedDate || "",
                                        bruto: (data[7] || "").replace(/[.,]/g, ""),
                                        retenido: (data[8] || "").replace(/[.,]/g, ""),
                                        pagado: (data[9] || "").replace(/[.,]/g, ""),
                                        estado: data[2] || "",
                                        path: href,
                                        oversea: oversea,
                                    });
                            ...
                            // Go back
                            await page.locator('body > div:nth-child(2) > center > center > form:nth-child(3) > table > tbody > tr > td:nth-child(3) > input[type=button]').click();
                            await page.waitForSelector(tableSelector);
                            hasNewLink = true;
                            break;
                        }
            ...
            await page.waitForTimeout(1000);
            await page.locator('body > div:nth-child(2) > center > table:nth-child(2) > tbody > tr:nth-child(6) > td > form > table > tbody > tr > td:nth-child(1) > div > font > input[type=button]').click();
            await page.waitForTimeout(1000);
        }
